...
- [x] **消息中心起步（站内信）**（2026-09-25 完成：store 契约 3 项 + gin API 集成 2 项全绿；`notifications` 收件箱（user_id 即安全边界、company_id 可选、已读幂等），首个事件源接设备申请审批流——提交通知管理员待办、通过/驳回回执申请人，投递失败不阻塞主流程；Web 顶栏铃铛 30s 轮询 + popover 收件箱。WebHook 出站通道已在 A4 落地）
- [ ] 员工自助门户 / 报表中心 / 软件许可 / 耗材管理
