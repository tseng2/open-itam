...
- [x] **多公司组织架构底座**
  ...
  - [x] 公司（多租户）管理闭环：新增/改名/删除（名称全局唯一 409、删除全业务表引用拦截 409 带明细、零引用硬删）、写面 admin，组织架构页实装（原 mock「东莞/苏州」写死废除）+ 人员总览真实数据。（2026-09-26 完成：company_api_test 契约锁死；总览大盘公司分布卡同步真实化）
- [x] **关系型数据库底座重构**
...
- [x] **非Agent外设录入与用友U8集成**
  ...
  - [x] 支持录入硬件资产关联的 **“用友 U8 采购订单号”**（仅台账明细字段，不作列表主字段）。（2026-09-26 拍板收口：**U8 仅作台账溯源字段，不做 API 集成**——CIYO 对标同口径；大盘「U8 采购追踪」假卡与设置页「U8 v18 集成」假 tab 移除，订单号在资产页可录/详情可见/全文搜索命中）
- [x] **UI 资产大盘升级**
  - [x] 资产列表、详情页、时间轴展示，支持按公司切分视图。
  - [x] 总览大盘全量真实化（2026-09-26 收官：`/api/v1/dashboard/summary` 全员轻聚合——资产状态计数/待处理变更/终端活跃失联/Agent 版本分布；mock 演示数字清零，详见 architecture.md 总览大盘契约）
- [x] **电脑台账（Excel）全表对齐**
  ...
  - [x] 移动存储领用表、配件记录表（模型与 API 阶段一就绪；2026-09-26 前端管理页补齐清欠——移动存储领用 / 配件出入库两页挂资产管理组，storage-lendings PUT 显式 null 清空契约修复并以 TDD 锁死）。
...
- [x] **数据可视化看板**
  - [x] 基于新模型的管理大盘与统计报表。（大盘：2026-09-26 总览页全量真实化；统计报表：P2 报表中心 8 卡/年度/月度/分布已落地）

## 📍 阶段三：软件合规与进阶采集

- [ ] **macOS 全量采集与支持**
  - [ ] 补齐 macOS Agent 采集与自动化部署机制。
- [x] **软件 License 管理与合规审计**（2026-09-26 完成：`software_pools` 受控池（读面登录可读写面 admin、名称同公司唯一、许可挂接做席位来源）+ `device_software` 终端软件清单结构化快照（full 上报每小时全量覆盖、公司归属取绑定资产）+ softwareaudit 合规引擎（每小时比对、超用站内信 24h 冷却去重、未受控清单只进报表）+ `/software-compliance` 合规报表（admin）+ 软件页三 tab；纯函数比对口径单源（精确/包含匹配 + 系统组件白名单防误伤），CIYO/Snipe-IT 均无此能力）
  - [x] 后台定义受控软件池。
  - [x] 结合 Agent 采集软件进行盗版/超用识别。
- [x] **Agent 自动更新**
